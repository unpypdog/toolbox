...
- 提交前校验，任一不满足即提示且**不发起请求**：
  - 非空，且能被 `new URL()` 解析
  - 协议必须是 `http:` 或 `https:`
  - 明文 `http:` 仅允许 `localhost` / `127.0.0.1`——因为页面的 CSP 只对回环地址放开 http，其他 http 会被浏览器直接拦掉，提前拒绝才能给出可操作的提示
  - 不接受含账号密码的地址（userinfo）——否则凭据会被写进 localStorage，违背「凭据不落盘」
  - 只接受「主机 + 端口」，不接受路径、查询串或片段。拼接口时会直接接在地址后面，带路径会变成 `/api/api/Login/login`
- **最常见的误操作是粘贴浏览器地址栏里的登录页地址**（形如 `https://主机:端口/login`）。它带路径，而且**端口通常与接口不同**——去掉路径也连不通。这种情况必须挡下并把原因说清楚，否则操作者只会反复试同一个地址。这条已在实现中单独处理并有专门的错误文案
- **只填对主机、端口仍填错**（例如去掉路径后留下登录页的端口）时，地址能通过全部校验，请求也确实发出去了，但会被浏览器的跨域检查拦下。此时前端服务会正常应答，只是不返回 `Access-Control-Allow-Origin`，因此**现象与"网络不通"完全一致**——浏览器出于安全考虑只抛一句 `Failed to fetch`，不告诉脚本真正原因（只有控制台能看到）。脚本无法区分，所以网络类错误的提示必须**优先怀疑地址填错**并带上实际尝试的地址，否则操作者会去排查网络、代理、证书，方向全错。实测：接口端口预检返回 204 + `Access-Control-Allow-Origin: *`，前端端口返回 200 且无任何 CORS 头
...
中文 Windows 上 Excel 另存为 CSV 时，默认用的是系统 ANSI 代码页（GBK），不是 UTF-8。而 `File.text()` **永远按 UTF-8 解码**，于是 GBK 文件的中文表头会变成乱码，页面报「缺少必要表头：姓名、手机号、单位名称」——使用者看到的却是文件明明没问题，极难自查。这是实际反馈中出现过的故障。

因此 CSV 不走 `File.text()`，改为读 `ArrayBuffer` 自行判断编码，顺序为：

1. 有 BOM 就按 BOM 指定的编码解（utf-8 / utf-16le / utf-16be）
2. 无 BOM 时先用 `TextDecoder("utf-8", { fatal: true })` 严格试解
3. 抛错则依次回退到 `gbk`、`big5`，同样用 `fatal`
4. 全部失败才报「无法识别文件编码」

两个关键取舍：

- **必须用 `fatal: true`。** 默认的宽松模式会把非法字节静默替换成 U+FFFD，GBK 文件会被"成功"解码成一堆问号，错误反而更难查。严格模式下非法序列直接抛错，才能逐级回退。
- **无法识别无 BOM 的 UTF-16。** 没有 BOM 时字节层面无法与单字节编码可靠区分，所以不猜。Excel 和记事本导出 UTF-16 时都会写 BOM，这条路径实际够用；真遇到无 BOM 的情况会得到明确的「无法识别文件编码」而不是误导性的「缺少表头」。

配套改动：

- 文件摘要里显示实际使用的编码（如 `74 B · 1 条 · GBK`），出问题时一眼能看出解码走对了没有
- 「缺少必要表头」的报错带上实际编码，避免再去猜
- 回归测试 Test 17 用 GBK 字节构造 CSV 并断言解析成功与摘要显示编码；已用变异测试验证：禁用 GBK 回退后该断言立即失败

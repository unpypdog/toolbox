...
12. **图文合并 = 维护一张带 `source` 列的数据表，按四步走**。
    ...
    **⚠ 这一条曾经把「文字优先」的强度改弱，导致线上事故，别再改弱第二次**：
    第 ③ 步里必须同时存在这两句，缺了就会出现「用户的日期被图片上的日期换掉」——
    - 「匹配到的字段一律以文字的值为准，**整字段替换**，不和图片值做任何调和」
      （禁止比较两个日期后挑一个、禁止把文字的月日与图片的年份拼起来）；
    - 「图片上的年份绝不能用来改文字里的年份」。
    真实事故：文字写 `26年1月12日`、图片是往期证书印着 `22年10月20日`，
    结果所有人的日期都变成 22 年。**只写「文字优先」是不够的** ——
    模型会把两边当同一份信息的两种说法去"合并"，挑年份更小的那个。
    还有一条相邻规则也别丢：文字的全局值**只填进该字段为空的行**，
    不覆盖图片里已有的值（否则局部信息会被扩大成整批覆盖）。
    提示词里**只描述流程、不描述中间产物**：数据表是内部推理，输出仍然只有
    `{"records":[...],"unreadable":""}`，多一个字段 `extractRecords` 就取不到数据。
    护栏在 `test_te_cert_cloud_core.js` 的 `[6]` 节「图文日期冲突」那一组：
    钉住整字段替换、禁止比年份、禁止拼接、全局值只填空字段，以及示例七的存在。
    改这一节的措辞时**必须同时改断言**，否则下次改弱没人拦得住。

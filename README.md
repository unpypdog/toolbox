...
两者是**同一次颁发的同一个名单**，不是两批人。AI 按四步处理，全程维护一张人员数据表
（列是 `name / hospital / date / note`，外加一列 `source` 标明这一行来自图片还是文字）：

1. **先看清结构** —— 分别确认图片和文字各能提供哪些字段，不预设图片只有姓名。
2. **解析图片填表** —— 逐行读取姓名、医院、日期，读不到的字段留空，保持图片原有行序。
3. **解析文字补到对应行** —— 按「明确点名的人 > 分组/范围 > 行列顺序 > 唯一全局值」
   的优先级逐人匹配。有多个医院或日期时**必须分别匹配**，绝不任选一个覆盖所有人；
   匹配到谁就只覆盖谁的同类字段，文字没涉及的图片字段保持原值。
4. **对完整表做分析再输出** —— 以这张表为唯一事实来源，输出阶段不许再凭印象补字段。

只有某字段在文字里**恰好只有一个值、且没有分组迹象**时，才作为全局值套用；医院和日期
分别判断。多个候选值归属不清时不猜：有图片原值就保留，没有就留空，并写进该行的
「AI 存疑」提示。
...
修改工具前请先读 `CLAUDE.md` 里这个工具的**十三条硬约束** —— 全部是踩过的坑，
包括模板载荷同步、CSP 白名单、Adobe 专属令牌端点、预检请求头限制、合批分页符、
AI 提示词里不许出现真实日期、「先建图片基础表再按作用域合并文字」这条合并规则，
